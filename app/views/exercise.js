...
export class ViewExercise extends View {
 ...
 // }
  ...
  onMount() {
    ...
    this.altitude = new Altitude("#subviewAltitude");
    ...
    this.hike = new Hike(this.elBoxStats);
  ...
  onRender() {
    if (exercise && exercise.stats) {
      ...
      this.lblSpeedUnits.text = `current ${speed.units}`;
      ...
      this.lblSpeedAvgUnits.text = `average ${speedAvg.units}`;

      const distance = utils.formatDistance(exercise.stats.distance);
      this.lblDistance.text = distance.value;
      this.lblDistanceUnits.text = `total ${distance.units}`;
